...
require_once $_SERVER['DOCUMENT_ROOT'] . '/vendor/itbz/fpdf/src/fpdf/FPDF.php';
...
class Reports extends Controller
{
    ...
    public function __construct($request)
    {
        ...
        $this->values['reports'] = $this->getFormatsReports();
    }
    ...
    public function makeReportAction()
    {
        ...
        if ($this->isAllow('acc')) {
            ...
            $this->idReport = $this->request['idReport'];
            $this->breakFields = explode(",", trim($this->values['config']['reports'][$this->idReport]['break_fields']));

            $this->query = $this->values['config']['reports'][$this->idReport]['query'];
            $formato = $this->values['config']['reports'][$this->idReport];
    ...
    private function getFormatsReports()
    {
        ...
        if (is_array($this->values['config']['reports'])) {
            $perfilUsuario = $_SESSION['CUP_PROFILE'];
            foreach ($this->values['config']['reports'] as $value) {
    ...
    {
        return $this->values['config']['reports'][$idReport]['title'];
    }

    public function getComment($idReport)
    {
        return $this->values['config']['reports'][$idReport]['comment'];
    }
    ...
    public function getFilters($idReport)
    {
        ...
        if (is_array($this->values['config']['reports'][$idReport]['filters'])) {
            foreach ($this->values['config']['reports'][$idReport]['filters'] as $index => $filter) {
    ...
    public function getPdf($parametros, $aditionalFilter = '')
    {
        ...
        if (is_array($this->filter['columnsSelected'])) {
            foreach ($this->filter['columnsSelected'] as $key => $column) {
                if ($this->filter['valuesSelected'][$key] != '') {
                    $entidad = $this->values['config']['reports'][$this->idReport]['filters'][$key]['entity'];
                    if ($entidad) {
                        $aux = explode(",", $entidad);
                        $entidad = $aux[0];
        ...
        $rows = DB::getInstance()->select($this->query);
        $nRegistros = count($rows);
...
class ListadoPDF extends \fpdf\FPDF
{
    ...
    /**
     * Decodifica una cadena de caracteres para quitar la codificación que el motor
     * de templates TWIG ha generado.
     *
     * @param string $variable La cadena de caracteres a ser analizada
     * @param integer $numc (Opcional) El numero de caracteres a devolver de la cadena $variable
     * @return string
     */
    public function DecodificaTexto($variable, $numc = 0)
    {

        $resultado = $variable;

        if ($numc > 0)
            $resultado = substr($resultado, 0, $numc);

        return $resultado;
    }

    /**
     * Devuelve la anchura de la página en la unidad de medida
     *
     * @return float
     */
    ...
    /**
     * Devuelve la anchura disponible de la página en la unidad de medida
     *
     * La anchura disponible es la real menos los márgenes derecho e izquierdo
     *
     * @return float
     */
    public function getPrintablePageWidth()
    {
        return $this->w - $this->lMargin - $this->rMargin;
    }

    /**
     * Calcula el alto que ocupará la línea y si no cabe, salta página
     *
     * @param float $lineHeight Altura básica de la linea
     * @param array $row Valores a imprimir
     * @param array $columnsMulticell Con las columnas MultiCell (width,field)
     */
    function CheckLinePageBreak($lineHeight, $row, $columnsMulticell)
    {
        $lineasMaximas = 0;
        foreach ($columnsMulticell as $column) {
            $lineasMaximas = max($lineasMaxima, $this->NbLines($column['width'], trim($row[$column['field']])));
        }
        $this->CheckPageBreak($lineasMaximas * $lineHeight);
    }
